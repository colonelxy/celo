...
export default function NFTDetails() {
    ...
    const MrktPlaceContract = useContract({
    ...
    async function fetchListing() {
        ...
        if(listingEntities.length ===0) {
            window.alert("Listing dies not exist or has been cancelled");
            return router.push("/");
        }

        const listing = listing[0];
        ...
        async function fetchNFTDetails() {
            ...
            async function updateListing() {
                setUpdating(true);
                const updateTxn = await MrktPlaceContract.updateListing(
                    nftAddress,
                    tokenId,
                    parseEther(newPrice)
                );
            ...
            async function cancelListing() {
                setCanceling(true);
                const cancelTxn = await MrktPlaceContract.cancelListing(
                    nftAddress,
                    tokenId
                );
                await cancelTxn.wait();
                window.alert("Listing canceled successfully");
                await router.push("/");
                await fetchListing();
                setCanceling(false);
            }
            ...
            async function buyListing() {
                setBuying(true);
                const buyTxn = await MrktPlaceContract.purchaseListing(
                    nftAddress,
                    tokenId,
                    {
                        value: listing.price,
                ...
                {loading ? (
                    ...
                    <div className={styles.contractInterface}>
                        ...
                        {isOwner && isActive && (
                            ...
                            className={styles.btn}>Cancel Listing</button>
                            </>
                        )}
                        {!isOwner && isActive && (
                            <button
